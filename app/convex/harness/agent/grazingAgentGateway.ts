 ...
 *
 * Usage:
 *   await ctx.runAction(api.harness.agent.grazingAgentGateway.agentGateway, {
...
function createRunStepRecorder(
  ...
  return {
    recordStep: async (payload) => {
      ...
      try {
        await ctx.runMutation(internal.harness.agent.agentAdmin.appendAgentRunStepInternal, {
          runId,
          farmExternalId,
...
export const agentGateway = action({
  ...
  handler: async (ctx: ActionCtx, args): Promise<{
    ...
    const runId = await ctx.runMutation(internal.harness.agent.agentAdmin.startAgentRunInternal, {
      farmExternalId: args.farmExternalId,
      trigger: args.trigger,
    ...
    if (args.trigger === 'morning_brief') {
      ...
      let planData = hasProvidedContext && providedPlanData
        ? providedPlanData
        : await ctx.runQuery(api.workflows.intelligence.getPlanGenerationData, {
            farmExternalId: args.farmExternalId,
            date: today,
          })
      ...
      if (args.dryRun && planData.existingPlanId && (!planData.pastures || !planData.settings || !planData.farm)) {
        const [settings, farm, mostRecentGrazingEvent, pastures] = await Promise.all([
          ctx.runQuery(api.workflows.intelligence.getSettingsForFarm, { farmExternalId: args.farmExternalId }),
          ctx.runQuery(api.workflows.intelligence.getFarmByExternalId, { externalId: args.farmExternalId }),
          ctx.runQuery(api.workflows.intelligence.getMostRecentGrazingEvent, { farmExternalId: args.farmExternalId }),
          ctx.runQuery(api.workflows.intelligence.getPasturesForFarm, { farmExternalId: args.farmExternalId }),
        ])
      ...
      if (planData.existingPlanId && !args.dryRun) {
        ...
        await ctx.runMutation(internal.harness.agent.agentAdmin.completeAgentRunInternal, {
          runId,
          status: 'blocked',
          toolCallCount: 0,
      ...
      if (!args.dryRun && harnessState.memories.length > 0) {
        await ctx.runMutation(internal.harness.agent.agentAdmin.touchMemoriesInternal, {
    ...
    if (args.trigger === 'observation_refresh') {
      const activeJob = await ctx.runQuery(api.workflows.satelliteFetchJobs.getActiveJob, {
        farmExternalId: args.farmExternalId,
      })
      const latestObservationDate = await ctx.runQuery(api.data.observations.getLatestObservationDate, {
        farmExternalId: args.farmExternalId,
      })
      await recorder.recordStep({
        stepType: 'info',
      ...
      const jobId = await ctx.runMutation(api.workflows.satelliteFetchJobs.createForManualRefresh, {
        farmExternalId: args.farmExternalId,
      })
      await recorder.recordStep({
        stepType: 'tool_call',
    ...
    if (args.trigger === 'plan_execution') {
      const todayPlan = await ctx.runQuery(api.workflows.intelligence.getTodayPlan, {
        farmExternalId: args.farmExternalId,
      })
      await recorder.recordStep({
        stepType: 'info',
