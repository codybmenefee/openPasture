...
async function runDailyPlanningAgent(
  ...
  if (!targetPaddockId) {
    const allPaddocks = await ctx.runQuery(api.harness.tools.grazingAgentTools.getAllPaddocksWithObservations, { farmExternalId })
  ...
  let forecast = isDryRun
    ? await ctx.runQuery(api.harness.tools.grazingAgentTools.getActiveForecast, {
        farmExternalId,
        paddockExternalId: resolvedPaddockId,
      })
    : await ctx.runMutation(api.harness.tools.grazingAgentTools.getOrCreateForecast, {
        farmExternalId,
        paddockExternalId: resolvedPaddockId,
      })
  ...
  if (forecast.forecastedSections.length === 0) {
    ...
    forecast = await ctx.runQuery(api.harness.tools.grazingAgentTools.getActiveForecast, {
      farmExternalId,
      paddockExternalId: resolvedPaddockId,
    })
  ...
  const principles = await ctx.runQuery(api.harness.tools.grazingAgentTools.getGrazingPrinciples, {
    farmExternalId,
  }) as GrazingPrinciples & { customRules: string[] }

  // 5. EVALUATE FORECAST CONTEXT
  const forecastContext = await ctx.runQuery(api.harness.tools.grazingAgentTools.evaluateForecastContext, {
    farmExternalId,
    paddockExternalId: resolvedPaddockId,
  })
  ...
  const currentForecast = await ctx.runQuery(api.harness.tools.grazingAgentTools.getActiveForecast, {
    farmExternalId,
    paddockExternalId: resolvedPaddockId,
  })
  ...
  if (targetPaddock.geometry) {
    try {
      ...
      try {
        const ndviGrid = await ctx.runAction(api.workflows.ndviGrid.generateNDVIGrid, {
          farmExternalId,
          paddockExternalId: resolvedPaddockId,
        })
        if (ndviGrid.hasData) {
...
Next section index: ${Math.min(currentForecast.activeSectionIndex + 1, currentForecast.forecastedSections.length - 1)}`
  ...
  for (const toolCall of toolCalls) {
    ...
    if (toolCall.toolName === 'getUngrazedRemaining') {
      try {
        const ungrazedResult = await ctx.runQuery(api.harness.tools.grazingAgentTools.getUngrazedRemaining, {
    ...
    if (toolCall.toolName === 'createDailyPlan') {
      try {
        ...
        createdPlanId = await ctx.runMutation(api.harness.tools.grazingAgentTools.createPlanWithSection, {
          farmExternalId,
        ...
        await ctx.runMutation(api.harness.tools.grazingAgentTools.finalizePlan, {
          farmExternalId,
        })
...
async function generateForecastSections(
  ...
  // Get paddock context
  const paddockContext = await ctx.runQuery(api.harness.tools.grazingAgentTools.getPaddockContextForAgent, {
    farmExternalId,
    paddockExternalId,
  })

  // Get forecast details
  const forecast = await ctx.runQuery(api.harness.tools.grazingAgentTools.getActiveForecast, {
    farmExternalId,
    paddockExternalId,
  })
    ...
    tools: {
      drawSection: tool({
        ...
        execute: async (args) => {
          ...
          try {
            const drawResult = await ctx.runMutation(api.harness.tools.grazingAgentTools.drawSection, {
              forecastId,
      ...
      getUngrazedRemaining: tool({
        ...
        execute: async () => {
          ...
          try {
            const remaining = await ctx.runQuery(api.harness.tools.grazingAgentTools.getUngrazedRemaining, {
              forecastId,
            })
