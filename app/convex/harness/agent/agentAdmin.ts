...
async function assertAgentDashboardAccessForAction(
  ...
  if (userExternalId === DEFAULT_USER_EXTERNAL_ID) {
    const devUser: Doc<'users'> | null = await ctx.runQuery(api.data.users.getUserByExternalId, {
  ...
  const user: Doc<'users'> | null = await ctx.runQuery(api.data.users.getUserByExternalId, {
...
export const simulateRun = action({
  ...
  handler: async (ctx, args): Promise<SimulateRunResult> => {
    ...
    const farm: Doc<'farms'> | null = await ctx.runQuery(api.workflows.intelligence.getFarmByExternalId, {
      externalId: args.farmExternalId,
    })
    if (!farm) throw new Error('Farm not found')

    const result = await ctx.runAction(api.harness.agent.grazingAgentGateway.agentGateway, {
