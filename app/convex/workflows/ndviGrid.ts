...
export const generateNDVIGrid = action({
  ...
  handler: async (ctx, args): Promise<NDVIGridResult> => {
    ...
    const pastureData = await ctx.runQuery(api.data.paddocks.getPastureByExternalId, {
      farmExternalId: args.farmExternalId,
      paddockExternalId: args.paddockExternalId,
    })
    ...
    const farm = await ctx.runQuery(api.data.farms.getByExternalId, {
    ...
    const availableDates = await ctx.runQuery(api.data.satelliteTiles.getAvailableDatesByExternalId, {
      farmExternalId: args.farmExternalId,
      tileType: 'ndvi',
    })
    ...
    // Get the NDVI tile
    const tile = await ctx.runQuery(api.data.satelliteTiles.getTileByExternalId, {
      farmExternalId: args.farmExternalId,
...
export const calculatePaddockNDVI = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    const settings = await ctx.runQuery(api.harness.tools.grazingAgentTools.getFarmSettings, {
      farmExternalId: args.farmExternalId,
    })
