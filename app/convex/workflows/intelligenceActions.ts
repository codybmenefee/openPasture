...
// All external calls should route through api.harness.agent.grazingAgentGateway.agentGateway
...
export const generateDailyPlan = action({
  ...
  handler: async (ctx, args): Promise<Id<'plans'> | null> => {
    ...
    const data = await ctx.runQuery(api.workflows.intelligence.getPlanGenerationData as any, {
      farmExternalId,
      date: today,
    ...
    const gatewayResult = await ctx.runAction(api.harness.agent.grazingAgentGateway.agentGateway, {
...
export const runDailyBriefGeneration = action({
  handler: async (ctx): Promise<Array<{ farmId: string; success: boolean; planId?: Id<'plans'>; error?: string }>> => {
    const farms = await ctx.runQuery(api.workflows.intelligence.getAllFarms as any) as any[]
...
export const cleanupFallbackPlans = action({
  ...
  handler: async (ctx, args): Promise<{ deleted: number }> => {
    ...
    const result = await ctx.runMutation(api.workflows.intelligence.deleteAllFallbackPlans, {
      farmExternalId,
    })
